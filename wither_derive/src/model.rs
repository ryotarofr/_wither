...
impl<'a> MetaModel<'a> {
    ...
    fn extract_collection_name(&mut self, meta: &syn::Meta) {
        let name = match meta {
            syn::Meta::NameValue(val) => match &val.value {
                ...
                _ => abort!(val.value, "this must be a string literal expression"),
            },
    ...
    fn extract_selection_criteria(&mut self, meta: &syn::Meta) {
        let fnpath = match meta {
            syn::Meta::NameValue(nv) => match &nv.value {
                syn::Expr::Lit(expr_lit) => match &expr_lit.lit {
                    syn::Lit::Str(s) => match syn::parse_str::<syn::Path>(&s.value()) {
                        Ok(path) => path,
                        Err(err) => abort!(
                            s,
                            "this must be a valid path string, e.g. `#[model(selection_criteria = \"my_mod::some_path\")]`";
                            hint = err
                        ),
                    },
                    _ => abort!(
                        expr_lit.lit,
                        "this must be a string literal, e.g. `selection_criteria = \"my_mod::some_path\"`"
                    ),
                },
                _ => abort!(
                    nv.value,
                    "this must be a string literal expression, e.g. `selection_criteria = \"my_mod::some_path\"`"
                ),
            },
            _ => abort!(meta, META_MUST_BE_KV_PAIR),
        };
        // Now that we've parsed the path, store it:
    ...
    fn parse_attrs(attrs: &[syn::Attribute], container_name: &str) -> Vec<syn::Meta> {
        attrs.iter()
            ...
            .filter_map(|attr| match attr.parse_args() {
                Ok(meta) => Some(meta),
            ...
            .fold(vec![], |mut acc, nested| {
                for inner in nested {
                    match inner {
                        NestedMeta::Meta(meta) => acc.push(meta),
                        NestedMeta::Lit(lit) => abort!(lit, "unexpected literal value"),
                    }
                }
                acc
            })
